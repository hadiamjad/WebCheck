...
async function insertResponseInfo(newHttpResp, website){ 
  const file = 'output/'+website+'/responsesInfo.json';
  jsonfile.writeFile(file, newHttpResp, { flag: 'a' }, function (err) {
    if (err) console.error(err);
  })
}

async function insertCookieTableActivity(newHttpResp, website){ 
  const file = 'output/'+website+'/cookieTableActivity.json';
  jsonfile.writeFile(file, newHttpResp, { flag: 'a' }, function (err) {
    if (err) console.error(err);
  })
}
...
app.post('/responseinfo', (req, res) => {
  insertResponseInfo(req.body, website[0]);
  res.send("request-success");
})

app.post('/cookietableactivity', (req, res) => {
  insertCookieTableActivity(req.body, website[0]);
  res.send("request-success");
})
