...
function onEvent(debuggeeId, message, params) {
    ...
    if (message == "Fetch.requestPaused"){
        const url = chrome.extension.getURL('surrogate.json');
        fetch(url)
            .then((response) => response.json())
            .then((json) => {
              var stmt = json;
              if (stmt.hasOwnProperty(params.request.url)){ 
                ajaxMe(params.request.url, params.request.headers, params.request.method, params.request.postData, (data) => {
                    ...
                    console.log("requestPaused", params.request.url);
                    fetch(chrome.extension.getURL(stmt[params.request.url])).then((response) => response.text()).then((resp) => {
                        data.response = resp;
                        console.log('requestContinued', resp);
