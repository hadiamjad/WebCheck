...
def main():
    ...
    for site in fold:
        try:
            ...
            # "num_method_successors-18",
            # "num_method_predecessors-19",
            # "descendant_of_storage_node-20",
            # "ascendant_of_storage_node-21",
            # "parent_method-node-id-22",
            # "parent_method-req-sent-23",
            # "descendant_of_fingerprinting-24",
            # "ascendant_of_fingerprinting-25",
            methods = {}
            ...
            try:
                ...
                for mthd in methd_dic.keys():
                    for key in storage.keys():
                        if methd_dic[mthd][0] + "@" + methd_dic[mthd][1] == key:
                        ...
                        # e.g 'Object._.db' in addEventList and '_.db' in network request
                            ...
                            methd_dic[mthd][0] == key.split("@")[0]
                            and methd_dic[mthd][1] in key.split("@")[1]
                        ):
                            # ["analytic", "track", "touchstart", "visibilitychange", "mousemove", "copy", "paste", "geolocation"]
                            fingerprinting_ids.append(int(mthd))
            except:
            ...
            for mthd in methods.keys():
                ...
                methods[mthd].append(0)
                if methods[mthd][22] != 0:
                    for node_id in nx.descendants(graph, str(methods[mthd][22])):
                        if int(node_id) in fingerprinting_ids:
                            methods[mthd][24] += 1
                    for node_id in nx.ancestors(graph, str(methods[mthd][22])):
                        if int(node_id) in fingerprinting_ids:
                            methods[mthd][25] += 1
